...
class DAO {
	...
	*/
	public function buscarData ()
	{
	 ...
	 $numfilas = count($tiendas);
					...
					{
					 $html = $html.'<a href="'.$tiendas[$x]['url'].'" target="_blank">'.'<img src="'.$tiendas[$x]['logo'].'" alt=""><center><p class="shop-description">Visitar: '.$tiendas[$x]['descripcion'].'</p>
	...
	public function buscarTiendaXFiltro ($filtroProducto, $filtroCategoria, $filtroMarca, $filtroCcomercial, $orderByPrecio)
	{
	 ...
	 {		
			$consulta="select distinct p.url, p.logo, p.descripcion, p.id_tienda, p.desc_video, p.categorias, p.url_categoria , p.marcas from 
						...
						SELECT  p.precio precio, t.url url, t.url url_categoria, t.logo logo, t.descripcion descripcion, t.id id_tienda, tv.desc_video desc_video, buscarCategoriaXtienda(t.id) categorias, buscarMarcaXtienda(t.id) marcas 
		 ...
		 { 
				$consulta = "select distinct p.url, p.logo, p.descripcion , p.id_tienda, p.desc_video ,p.categorias	, p.url_categoria , p.marcas
					from 
					(SELECT p.precio, t.url url, t.logo, t.descripcion, t.id id_tienda, pc.id_categoria,
		...
		// Si solamente esta diligenciado el filtro de marca:	
		 if($filtroMarca <> null and $filtroProducto == null and $filtroCategoria == null)
		  {
				$consulta  = "SELECT distinct t.url url, t.logo, t.descripcion ,tv.desc_video, buscarCategoriaXtienda(t.id) categorias, t.url url_categoria	, buscarMarcaXtienda(t.id) marcas
							   FROM productoccali p
						  join producto_tienda pt on pt.id_producto = p.id";
	...
	*/
	...
	/*
		consultar para el webservice
		Con esta consulta podemos exponer cualquier tabla de la base de datos
	*/
	...
	public function GetUsuarioTienda($idUsuario){
		
		$query = 'SELECT t.* FROM usuario_tiendaccali ut
					inner join tiendaccali t on t.id = ut.id_tiendaccali
	...
	public function GetUsuario($tabla,$arrayParametros){
			$dataUsuario = null;
			//Traer el usuario
			$usuario = $this->GetData('usuario',$arrayParametros);
			
			//traer las tiendas a las cuales pertenece
			$tiendas = $this->GetUsuarioTienda($usuario[0]['id']);
